...
function filterSuggestions (query, suggestions, length) {
  ...
  return suggestions.filter((item) => regex.test(item.title)).slice(0, length)
}
    ...
    const options = this.state.options.map((item, i) => {
      ...
      return (
        React.createElement( 'li', {
          ...
          React.createElement( 'span', { dangerouslySetInnerHTML: markIt(item.title, this.props.query) })
        )
      )
    })
