...
function filterSuggestions (query, suggestions, length) {
  ...
  return suggestions.filter(function (item) { return regex.test(item.title); }).slice(0, length)
}
    ...
    var options = this.state.options.map(function (item, i) {
      ...
      return (
        React.createElement( 'li', {
          ...
          React.createElement( 'span', { dangerouslySetInnerHTML: markIt(item.title, this$1.props.query) })
        )
      )
    })
